...
(function () {
  ...
  fetch(searchUrl)
    ...
    .catch(function (err) {
      ...
      // 尝试备用路径
      var fallbacks = ['/search.json', '/geology-atlas/search.json']
        .filter(function (p) { return p !== searchUrl; });
      tryNext(0);
    ...
    if (resultsContainer) {
      if (results.length === 0) {
        ...
        if (noResultsHint) {
          ...
          if (searchMode === 'exact') {
            noResultsHint.querySelector('.empty-state__text').textContent =
              '标题中没有找到匹配，试试切换到「模糊搜索」';
          } else {
            noResultsHint.querySelector('.empty-state__text').textContent =
              '没有找到匹配的结果，试试其他关键词';
          }
        ...
        resultsContainer.innerHTML = results.map(function (item) {
          return '<article class="card" data-category="' + (item.category || '') + '">' +
            ...
            '<img src="' + (item.thumbnail || '/assets/images/placeholder.svg') + '" alt="' + item.title + '" loading="lazy" />' +
